...
// Ties break toward the harder mode: standard > situp > noob/manual. A missed
...
export function pickDayIcon(logs: PushupLogLike[], dailyTarget: number): DayIcon {
  ...
  for (const log of logs) {
    total += log.count;
    if (log.mode === 'standard') stdTotal += log.count;
...
export function dayIconToMode(icon: DayIcon): 'standard' | 'situp' | 'noob' | 'manual' {
  switch (icon) {
